Template.addUser2Role.events({ 
    'submit form': function(e) {
        ...
        if (rolUser==1){
            user2Role.insert({rol: userRole.findOne({nameC: 'ADMINISTRATOR'})._id, user: Meteor.users.findOne({_id: userId2Up})});
        }
        if (rolUser==2){
            user2Role.insert({rol: userRole.findOne({nameC: 'RESTORER'})._id, user: Meteor.users.findOne({_id: userId2Up})});
        }
		//}
        return false;
    }
